...
//pub type Owned<N, R, C = U1> = <InitializedTag as GenericOverInitializedness<N, R, C>>::Owned;
//pub type Owned<N, R, C = U1> = <DefaultAllocator as Allocator<N, R, C>>::Buffer;
pub type Owned<N, R, C = U1, G = InitializedTag> = <G as GenericOverInitializednessAllocatorDispatch<N, R, C, DefaultAllocator>>::Owned;

/// The row-stride of the owned data storage for a buffer of dimension `(R, C)`.
pub type RStride<N, R, C = U1, G = InitializedTag> =
    <<DefaultAllocator as Allocator<N, R, C>>::Buffer as Storage<N, R, C, G>>::RStride;
...
unsafe impl<N: Scalar, R: Dim, C: Dim, A: Allocator<N, R, C>> GenericOverInitializednessAllocatorDispatch<N, R, C, A> for UninitializedTag {
    type Owned = A::UninitBuffer;
}
unsafe impl<N: Scalar, R: Dim, C: Dim, A: Allocator<N, R, C>> GenericOverInitializednessAllocatorDispatch<N, R, C, A> for InitializedTag {
    type Owned = A::Buffer;
}
...
pub unsafe trait Storage<N: Scalar, R: Dim, C: Dim = U1, G: GenericOverInitializedness<N, R, C> = InitializedTag>: Debug + Sized {
    ...
    #[inline]
    unsafe fn get_address_unchecked_linear(&self, i: usize) -> *const G::Scalar {
        self.ptr().wrapping_offset(i as isize)
    }

    /// Gets the address of the i-th matrix component without performing bound-checking.
    #[inline]
    unsafe fn get_address_unchecked(&self, irow: usize, icol: usize) -> *const G::Scalar {
        self.get_address_unchecked_linear(self.linear_index(irow, icol))
    }

    /// Retrieves a reference to the i-th element without bound-checking.
    #[inline]
    unsafe fn get_unchecked_linear(&self, i: usize) -> &G::Scalar {
        &*self.get_address_unchecked_linear(i)
    }

    /// Retrieves a reference to the i-th element without bound-checking.
    #[inline]
    unsafe fn get_unchecked(&self, irow: usize, icol: usize) -> &G::Scalar {
    ...
    #[inline]
    fn as_slice(&self) -> &[G::Scalar];

    /// Builds a matrix data storage that does not contain any reference.
    fn into_owned(self) -> G::Owned
    where DefaultAllocator: Allocator<N, R, C>, G: GenericOverInitializednessAllocatorDispatch<N, R, C, DefaultAllocator>;

    /// Clones this data storage to one that does not contain any reference.
    fn clone_owned(&self) -> G::Owned
    where DefaultAllocator: Allocator<N, R, C>, G: GenericOverInitializednessAllocatorDispatch<N, R, C, DefaultAllocator>;
}
...
pub unsafe trait StorageMut<N: Scalar, R: Dim, C: Dim = U1, G: GenericOverInitializedness<N, R, C> = InitializedTag>: Storage<N, R, C, G> {
    /// The matrix mutable data pointer.
    fn ptr_mut(&mut self) -> *mut G::Scalar;

    /// Gets the mutable address of the i-th matrix component without performing bound-checking.
    #[inline]
    unsafe fn get_address_unchecked_linear_mut(&mut self, i: usize) -> *mut G::Scalar {
        self.ptr_mut().wrapping_offset(i as isize)
    }

    /// Gets the mutable address of the i-th matrix component without performing bound-checking.
    #[inline]
    unsafe fn get_address_unchecked_mut(&mut self, irow: usize, icol: usize) -> *mut G::Scalar {
        let lid = self.linear_index(irow, icol);
        self.get_address_unchecked_linear_mut(lid)
    }

    /// Retrieves a mutable reference to the i-th element without bound-checking.
    unsafe fn get_unchecked_linear_mut(&mut self, i: usize) -> &mut G::Scalar {
    ...
    #[inline]
    unsafe fn get_unchecked_mut(&mut self, irow: usize, icol: usize) -> &mut G::Scalar {
    ...
    #[inline]
    fn as_mut_slice(&mut self) -> &mut [G::Scalar];
}
...
pub unsafe trait ContiguousStorage<N: Scalar, R: Dim, C: Dim = U1, G: GenericOverInitializedness<N, R, C> = InitializedTag>:
    Storage<N, R, C, G>
{
}
...
/// `.get_unchecked_linear` returns one of the matrix component. This trait is unsafe because
/// failing to comply to this may cause Undefined Behaviors.
pub unsafe trait ContiguousStorageMut<N: Scalar, R: Dim, C: Dim = U1, G: GenericOverInitializedness<N, R, C> = InitializedTag>:
    ContiguousStorage<N, R, C, G> + StorageMut<N, R, C, G>
{
}
